import request from 'request';
const { _res, _body } = request({
  uri: `https://petstore.swagger.io/v2/pet/${id}`,
  method: 'get',
});
let _get = _body;
const newPet = _get;
const { _res2, _body2 } = request({
  uri: 'https://petstore.swagger.io/v2/pet/1234',
  method: 'get',
});
const { _res3, _body3 } = request({
  uri: 'https://petstore.swagger.io/v2/pet/abcd',
  method: 'get',
});
const { _res4, _body4 } = request({
  uri: `https://petstore.swagger.io/v2/pet/${id}`,
  method: 'get',
});
let _get4 = _body4;
_get4.name;
const { _res5, _body5 } = request({
  body: JSON.stringify(newPet),
  uri: 'https://petstore.swagger.io/v2/pet',
  method: 'put',
  contentType: 'application/json',
  json: true,
});
const { _res12, _body12 } = request({
  uri: `https://petstore.swagger.io/v2/pet/${id}`,
  method: 'get',
});
let _get6 = _body12;
const { _res6, _body6 } = request({
  body: JSON.stringify(_get6),
  uri: 'https://petstore.swagger.io/v2/pet',
  method: 'put',
  contentType: 'application/json',
  json: true,
});
const { _res7, _body7 } = request({
  body: newPet,
  uri: 'https://petstore.swagger.io/v2/pet',
  method: 'post',
  contentType: 'application/json',
  json: true,
});
const { _res8, _body8 } = request({
  formData: image,
  uri: `https://petstore.swagger.io/v2/pet/${id}/uploadImage`,
  method: 'post',
  contentType: 'multipart/form-data',
});
const { _res9, _body9 } = request({
  uri: `https://petstore.swagger.io/v2/pet/${id}`,
  method: 'delete',
});
const { _res10, _body10 } = request({
  uri: 'https://petstore.swagger.io/v2/pet/findByStatus',
  method: 'get',
  qs: {
    status: 'available',
  },
});
let _get5 = _body10;
const pets = _get5;
const { _res11, _body11 } = request({
  body: {
    id: 0,
    petId: 0,
    quantity: 0,
    shipDate: '2017-10-03T06:03:36.447Z',
    status: 'placed',
    complete: false,
  },
  uri: 'https://petstore.swagger.io/v2/store/order',
  method: 'post',
  contentType: 'application/json',
  json: true,
});
let _post3 = _body11;
const orderId = _post3;
